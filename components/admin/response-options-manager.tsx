...
export function ResponseOptionsManager() {
  ...
  if (loading) {
    return <div className="text-center py-8">Cargando opciones de respuesta...</div>
  }
      ...
      <div className="flex justify-between items-center">
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent>
            ...
            <OptionForm option={editingOption} onSave={handleSaveOption} onCancel={() => setIsDialogOpen(false)} />
          </DialogContent>
        </Dialog>
      </div>
          ...
          <Card key={option.id}>
            <CardHeader>
              <div className="flex justify-between items-start">
                <div>
                  <CardTitle className="text-lg">{option.option_text}</CardTitle>
                  <div className="flex gap-2 mt-2">
                    <Badge variant="outline">Puntos: {option.points}</Badge>
                    <Badge variant={option.excludes_from_calculation ? "destructive" : "default"}>
                      {option.excludes_from_calculation ? "Excluida del cálculo" : "Incluida en cálculo"}
                    </Badge>
                  </div>
                </div>
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <Input
          ...
          onChange={(e) => setFormData((prev) => ({ ...prev, option_text: e.target.value }))}
          required
        />
      </div>
      ...
      <div className="flex items-center space-x-2">
        <Switch
          ...
          onCheckedChange={(checked) => setFormData((prev) => ({ ...prev, excludes_from_calculation: checked }))}
        />
        <Label htmlFor="excludes_from_calculation">Excluir del cálculo de puntuación</Label>
      </div>
